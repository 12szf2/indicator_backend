...
/**
 * Sends a bug report email to the given recipient.
 ...
 */
export async function sendBugReportEmail(to, bugReportData, reporterInfo) {
  const { title, description, severity, stepsToReproduce, pageUrl, userAgent } = bugReportData;
  const { name, email } = reporterInfo;

  const subject = `[Hibabejelentés] - ${severity} - ${title}`;
...
${userAgent || "Nem adta meg"}`;
  ...
  try {
    ...
    console.log(`Bug report email sent to ${to}`);
  } catch (error) {
    console.error("Error sending bug report email to", to, ":", error);
