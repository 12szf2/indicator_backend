...
export class ServiceCache {
  ...
  async invalidate(...patterns) {
    const promises = patterns.map((pattern) => {
      ...
      return cache.delByPattern(fullPattern);
    });
  ...
  async invalidateRelated(operation, ...params) {
    switch (operation) {
      ...
      case "delete":
        ...
        await this.invalidate("all.*", "byYear.*", "alapadatok_id.*", "alapadatok_id_year.*", "count.*", "all_with_year_ordered.*", "alapadatok_with_year_ordered.*");
        if (params[0]) {
  ...
  async invalidateByAlapadatokId(alapadatokId) {
    ...
    await this.invalidate("all.*", "count.*", "all_with_year_ordered.*", "alapadatok_with_year_ordered.*"); // Also invalidate general lists
  }
  ...
  async invalidateByYear(year) {
    ...
    await this.invalidate("all.*", "all_with_year_ordered.*", "alapadatok_with_year_ordered.*"); // Also invalidate general lists
  }
