...
const serviceCache = new ServiceCache("log");

// Custom TTLs for logs (shorter than default since logs update frequently)
const LOG_CACHE_TTL = {
  LIST: 30 * 1000, // 30 seconds for log lists
...
const processLogQueue = withPerformanceMonitoring(
  async function processLogQueue() {
    ...
    try {
      ...
      // If this fails, we need to retry a few times
      queueRetryCount++;

      if (queueRetryCount <= MAX_RETRY_ATTEMPTS) {
        // Put the logs back at the front of the queue and retry in a few seconds
        logQueue.unshift(...logsToProcess);
        queueTimer = setTimeout(() => {
          processLogQueue();
          queueTimer = null;
        }, 2000 * queueRetryCount); // Exponential backoff
      } else {
        console.error(
          `Failed to process log queue after ${MAX_RETRY_ATTEMPTS} attempts. Dropping ${logsToProcess.length} logs.`
        );
        queueRetryCount = 0;
      }
    }
 ...
 */
export const logRequest = withPerformanceMonitoring(
 ...
 */
export const getLogsWithPagination = withPerformanceMonitoring(
  async function getLogsWithPagination(options = {}) {
    const {
      skip = 0,
      take = 50,
      userId,
 ...
 */
export const getLogById = withPerformanceMonitoring(
 ...
 */
export const getLogsByUserId = withPerformanceMonitoring(
 ...
 */
export const clearOldLogs = withPerformanceMonitoring(
  async function clearOldLogs(cutoffDate, level) {
    const where = {
      createdAt: {
        lt: cutoffDate,
      },
 ...
 */
export const getLogStats = withPerformanceMonitoring(
  async function getLogStats(since) {
